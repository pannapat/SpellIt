...
preparetorule (x:xs) = [initialsortandgroup x] ++ (preparetorule xs) 
    where
        ...
        initialsortandgroup [] = [] 
        initialsortandgroup xs = L.groupBy (\(a,b,c) (d,e,f) -> b == e) 
                                 (L.sortOn (\ys -> case ys of (a,PrePfxHead b _ _,c) -> b 
                                                              (a,PreSfxHead b _ _,c) -> b) xs)
       -------------------------------- 
...
firstnexttorule (x:xs) = [largeinitialsort x] ++ (firstnexttorule xs)
    where 
        ...
        initialsort [] = []
        initialsort xs = L.sortOn (\zs -> case zs of (a,b,PrePfx _ _ c _) -> c
                                                     (a,b,PreSfx _ _ c _) -> c) (L.sortOn (\ys -> case ys of (a,b,PrePfx _ _ _ c) -> c 
                                                                                                             (a,b,PreSfx _ _ _ c) -> c) xs)
        --------------------------------  
...
secondnexttorule (x:xs) = [largeinitialgroup x] ++ (secondnexttorule xs)
    where
        ...
        initialgroup xs = if (L.any (isPrePfx) xs)  
                              then L.groupBy (\(a,b,PrePfx _ _ c d) (e,f,PrePfx _ _ g h) -> (c == g && d == h)) xs
                              else L.groupBy (\(a,b,PreSfx _ _ c d) (e,f,PreSfx _ _ g h) -> (c == g && d == h)) xs   
        --isPrePfx
        isPrePfx :: (String,PreRuleHeader,PreRule) -> Bool
        isPrePfx (a,b,PrePfx _ _ _ _) = True
        isPrePfx (a,b,PreSfx _ _ _ _) = False             
        --------------------------------
...
fourthnexttorule (x:xs) (y:ys) = [L.map (L.map (L.map (\zs -> case zs of (a,PrePfxHead b c d,e,f) -> (a,PrePfxHead b c d,e,PfxHead b c d y,f)
...
torule [] = []
torule xs = L.map (L.last) (L.concat (L.concat (L.map (L.map (L.map (L.map (\(a,b,c,d,e) -> (d,e))))) xs)))

--sorttorule -> To sort the results of torule
--by Rule.
sorttorule :: [(RuleHeader,Rule)] -> [(RuleHeader,Rule)]
sorttorule [] = []
sorttorule xs = L.sortOn (\ys -> case ys of (PfxHead _ a _ _,b) -> a
                                            (SfxHead _ a _ _,b) -> a) xs

--grouptorule -> To group the results of sorttorule.
grouptorule :: [(RuleHeader,Rule)] -> [[(RuleHeader,Rule)]]
grouptorule [] = []
grouptorule xs = if (L.any (isPfx) xs)
                     then L.groupBy (\(PfxHead _ a _ _,b) (PfxHead _ c _ _,d) -> (a == c)) xs
                     else L.groupBy (\(SfxHead _ a _ _,b) (SfxHead _ c _ _,d) -> (a == c)) xs 
    where
        --isPfx
        isPfx :: (RuleHeader,Rule) -> Bool
        isPfx (PfxHead _ _ _ _,b) = True
        isPfx (SfxHead _ _ _ _,b) = False 
...
--secondfinaltorule 
secondfinaltorule :: [[[String]]] -> [[String]]
secondfinaltorule [] = []
secondfinaltorule xs = singlechunker (concatenate xs) 
    where
        --Nested Function Definitions.--
        --singlechunker
...
jsonparse (x:xs) (tempnamed,temphd) = do
    ...
    jsondecoded <- (A.decode <$> (LBSC.readFile x)) :: IO (Maybe Paradigm)
...
main = do
    ...
    case cmdargs of
        ...
        allargs -> do --Create a temporary file.
                      ...
                      print pfxsfxadder
                      let topreruler = toprerule pfxsfxadder
                      print topreruler
                      let simplifiedtopreruler = simplifytoprerule topreruler
                      print simplifiedtopreruler
                      let finalsimplifiedtopreruler = finalsimplifytoprerule simplifiedtopreruler
                      print finalsimplifiedtopreruler
                      let wordandpreruler = wordandprerule finalsimplifiedtopreruler
                      print wordandpreruler
                      let preparetoruler = preparetorule wordandpreruler
                      print preparetoruler
                      let firstnexttoruler = firstnexttorule preparetoruler
                      print firstnexttoruler
                      let secondnexttoruler = secondnexttorule firstnexttoruler
                      print secondnexttoruler
                      let thirdnexttoruler = thirdnexttorule secondnexttoruler
                      print thirdnexttoruler
                      let fourthnexttoruler = fourthnexttorule thirdnexttoruler (thirdrulelengthmapper thirdnexttoruler)
                      print fourthnexttoruler
                      let toruler = torule fourthnexttoruler
                      print toruler
                      let sorttoruler = sorttorule toruler
                      print sorttoruler
                      let grouptoruler = grouptorule sorttoruler
                      print grouptoruler
                      let prefinalruler = prefinalrule grouptoruler
                      print prefinalruler
                      let finalruler = finalrule prefinalruler
                      print finalruler
                      let firstfinalruler = firstfinalrule finalruler
                      print firstfinalruler
                      let secondfinalruler = secondfinaltorule firstfinalruler
                      print secondfinalruler
{----------------}
